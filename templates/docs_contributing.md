...
[pull requests](https://github.com/{{organization}}/{{repository}}/pulls).
...
- Document new features. {%- if has_changelog_and_upgrade_guide %}
  For user-facing changes, add a changelog entry; for breaking changes,
  update the upgrade guide.
  For details, see {ref}`maintaining-changelog-upgrade-guide`. {%- endif %}
...
you can pass {code}`-DBUILD_MQT_{{name.upper()}}_TESTS=OFF` to the CMake
configure step.
:::
...
pass {code}`-DBUILD_MQT_{{name.upper()}}_BINDINGS=ON` to the CMake configure
step.
...
{code}`python/mqt/{{repository}}` directory.
...
MQT {{name}} adheres to [Semantic Versioning],
...
Use it to explain how users may need to adapt their usage of MQT {{name}},
...
When it is time to release a new version of MQT {{name}},
...
changes for MQT {{name}}, you can remove the {code}`minor` or {code}`major`
label from the PR.
