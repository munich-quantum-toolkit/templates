...
- Sources: `docs/` {%- if project_type == "c++-mlir-python" %}
- Build MLIR docs: `cmake --build --preset release --target mlir-doc` {%- endif
  %}
...
- `clang-format` and `clang-tidy` for formatting/linting (see `.clang-format`
  and `.clang-tidy`) {%- if project_type == "c++-mlir-python" %}
- LLVM 22.1+ for building MLIR code
- GoogleTest for unit tests (located in `test/` and `mlir/unittests/`) {%- elif
  project_type == "c++-python" %}
- GoogleTest for unit tests (located in `test/`) {%- endif %}

{%- endif %}

### Python

- Python 3.10+ {%- if project_type in ["c++-python", "c++-mlir-python"] %}
- Stable ABI wheels for 3.12+; free-threading support for 3.14+
- `scikit-build-core` as build backend
- `nanobind` for bindings {%- endif %}
...
- `sphinx`
- MyST (Markdown)
- Furo theme {%- if project_type in ["c++-python", "c++-mlir-python"] %}
- `breathe` for C++ API docs {%- endif %}
...
- MUST update `CHANGELOG.md` and `UPGRADING.md` when changes are user-facing,
  breaking, or otherwise noteworthy. {%- endif %}
...
- PREFER C++20 STL features over custom implementations. {%- if project_type ==
  "c++-mlir-python" %}
- PREFER LLVM data structures and methods in `mlir/` (`llvm::SmallVector`,
  `llvm::function_ref`, etc.) over the STL. {%- endif %}
...
- Are all changes covered by at least one automated test? {%- if project_type in
  ["c++-python", "c++-mlir-python"] %}
- Were Python stubs regenerated via `uvx nox -s stubs` if bindings were
  modified? {%- endif %} {%- if has_changelog_and_upgrade_guide %}
