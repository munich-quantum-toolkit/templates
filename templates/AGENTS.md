...
- NEVER edit `.pyi` files in `python/mqt/{{ repository }}/` manually;
...
- PREFER LLVM data structures and methods in `mlir/`
  (`llvm::SmallVector`, `llvm::function_ref`, etc.) over the STL. {%- endif %}
...
- Were Python stubs regenerated via `uvx nox -s stubs`
  ...
  {%- endif %} {%- if has_changelog_and_upgrade_guide %}
